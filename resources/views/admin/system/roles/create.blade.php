...
@section('content')
<div class="admin-main__inner" x-data="roleForm()">
    ...
    <form method="POST" action="{{ route('admin.system.roles.store') }}">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem">
            <section class="admin-panel-card">
                ...
                <div class="admin-form-group">
                    ...
                    <input class="admin-input" name="color" value="{{ old('color') }}" placeholder="e.g. #e8773a" style="max-width:150px">
                </div>
                <div class="admin-form-group">
                    <label class="admin-label">Icon</label>
                    <input class="admin-input" name="icon" value="{{ old('icon') }}" placeholder="e.g. users">
                </div>
            </section>
            ...
            <section class="admin-panel-card">
                ...
                <div class="admin-form-group">
                    ...
                    <input class="admin-input" name="landing_page" value="{{ old('landing_page') }}" placeholder="admin.dashboard">
                </div>
                <div class="admin-form-group">
                    <label class="admin-label">Priority</label>
                    <input class="admin-input" name="priority" type="number" value="{{ old('priority', 0) }}" style="max-width:120px">
                </div>
                <div class="admin-form-group" style="display:flex;gap:1rem">
                    <label><input type="checkbox" name="is_active" value="1" checked> Active</label>
                    <label><input type="checkbox" name="is_default" value="1"> Default Role</label>
                </div>
                <div class="admin-form-group">
                    <label class="admin-label">Clone Permissions From</label>
                    <select class="admin-input" name="clone_from">
        ...
        <section class="admin-panel-card">
            <div class="admin-section-head">
                <div><h3>Permission Matrix</h3><p>Toggle permissions for this role. Super Admin access is automatic.</p></div>
            </div>
                        ...
                        <div class="admin-permission-group__items">
                            @foreach ($group['permissions'] as $perm)
                                <label class="admin-permission-item">
                                    <input type="checkbox" name="permissions[{{ $perm['id'] }}]" value="1" class="perm-{{ $groupKey }}">
                                    <span>{{ $perm['name'] }}</span>
        ...
        <div style="display:flex;gap:0.75rem;margin-top:1.5rem">
            <button class="button button--small" type="submit">Create Role</button>
