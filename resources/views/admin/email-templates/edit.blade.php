...
<div class="admin-main__inner">
    <section class="admin-topbar">
        <div>
            ...
            <p>Update your email template content and settings</p>
        </div>
        <a href="{{ route('admin.email-templates.index') }}" class="button button--small button--ghost">&larr; Back</a>
    </section>

    <form action="{{ route('admin.email-templates.update', $template) }}" method="POST" class="admin-edit-form">
        @csrf
        @method('PUT')
            ...
            <div class="admin-settings-section">
                ...
                <div class="admin-form-grid">
                    <label class="admin-form-grid__full">
                        <span>Name *</span>
                        <input type="text" name="name" value="{{ old('name', $template->name) }}" required class="w-full border rounded px-3 py-2 @error('name') border-red-500 @enderror">
                    ...
                    <label>
                        ...
                        <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $template->is_active)) style="accent-color:#0f8b8d;width:1.25rem;height:1.25rem;">
                    </label>
                    <label class="admin-form-grid__full">
                        <span>Subject *</span>
                    ...
                    <label class="admin-form-grid__full">
                        <span>HTML Body *</span>
                        <textarea name="body_html" rows="15" required class="w-full border rounded px-3 py-2 font-mono text-sm @error('body_html') border-red-500 @enderror">{{ old('body_html', $template->body_html) }}</textarea>
        ...
        <div style="display:flex;gap:1rem;padding-top:1rem;">
            <button type="submit" style="background:linear-gradient(135deg,#10b981 0%,#059669 100%);color:white;padding:0.75rem 2rem;border:none;border-radius:0.375rem;cursor:pointer;font-weight:600;font-size:0.9375rem;">Update Template</button>
