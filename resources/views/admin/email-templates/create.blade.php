...
<div class="builder-container">
    <div class="builder-sidebar">
        ...
        <div class="builder-sidebar-tabs">
            ...
            <button class="builder-tab-btn" onclick="switchTab('starters')">Starters</button>
        ...
        <form id="templateSaveForm" action="{{ route('admin.email-templates.store') }}" method="POST" style="display:flex; flex-direction:column; height:calc(100% - 40px); margin:0;">
            @csrf
            ...
            <div class="builder-sidebar-content">
                ...
                <div id="pane-meta" class="builder-tab-pane active">
                    ...
                    <div class="form-group-builder">
                        ...
                        <input type="text" name="name" id="template_name" value="{{ old('name') }}" placeholder="e.g. eBook Welcome Campaign" required>
                    </div>
                    <div class="form-group-builder">
                        <label>Type *</label>
                        <select name="type" id="template_type" required>
                            ...
                            @foreach($types as $t)
                            <option value="{{ $t }}" @selected(old('type') === $t)>{{ $typeLabels[$t] ?? ucfirst(str_replace('_', ' ', $t)) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group-builder">
                        ...
                        <div class="dropdown-var-wrapper">
                            <input type="text" name="subject" id="template_subject" value="{{ old('subject') }}" placeholder="Enter email subject line..." required>
                    ...
                    <div class="form-group-builder" style="display:flex; align-items:center; gap:0.5rem;">
                        ...
                        <input type="checkbox" name="is_active" value="1" id="template_is_active" checked style="width:1.25rem; height:1.25rem; accent-color:#0b7a75;">
                        <label for="template_is_active" style="margin:0;">Active (Ready for sending)</label>
                    </div>
                </div>

                <!-- Starters Pane -->
                <div id="pane-starters" class="builder-tab-pane">
                    <h3 style="font-size:0.9375rem; margin-top:0; color:#1e293b;">Choose Starter Template</h3>
                    <p style="font-size:0.75rem; color:#64748b; margin-top:-0.5rem; margin-bottom:1rem;">Selecting a starter template will overwrite your current draft canvas.</p>
                    
                    @foreach($starterTemplates as $key => $tpl)
                    <div class="starter-list-item" onclick="loadStarterTemplate('{{ $key }}')">
                        <strong style="font-size:0.8125rem; display:block; color:#1e293b;">{{ $tpl['name'] }}</strong>
                        <span style="font-size:0.75rem; color:#64748b;">{{ $tpl['description'] }}</span>
                    </div>
                    @endforeach
                </div>
            ...
            <div style="padding:1.25rem; border-top:1px solid #e2e8f0; display:flex; gap:0.75rem; background:#ffffff;">
                <button type="button" class="button" style="background:#0b7a75; color:white; flex:1; font-weight:600;" onclick="submitTemplateForm()">Create Template</button>
...
<script>
    ...
    const starterTemplates = @json($starterTemplates);
    ...
    let templateState = {
        ...
        blocks: [
            { id: 'logo_init', type: 'logo', properties: { alignment: 'center', width: '150', paddingTop: '20', paddingBottom: '15' } },
            { id: 'heading_init', type: 'heading', properties: { text: 'Relocation Assistance Welcome!', fontSize: '24px', color: defaultTheme.primaryColor, fontWeight: 'bold', alignment: 'center', paddingTop: '15', paddingBottom: '15', margin: '0' } },
            { id: 'text_init', type: 'text', properties: { text: 'Welcome to your custom visual email templates builder. Click on Starters to choose a layout, or click on Blocks to add headings, text paragraphs, buttons, dividers, and spacers.', fontSize: '16px', color: defaultTheme.textColor, fontWeight: 'normal', alignment: 'left', lineHeight: '1.6', paddingTop: '10', paddingBottom: '10' } },
            { id: 'footer_init', type: 'footer', properties: { background: '#f5f0e8', color: '#607080', padding: '25', alignment: 'center' } }
        ]
    };
    ...
    function loadStarterTemplate(key) {
        ...
        if (starter) {
            adminModal.confirm({
                ...
                templateState.blocks = JSON.parse(JSON.stringify(starter.blocks)); // deep clone
                ...
                // Switch type selector to match if visible
                if (key === 'professional_download') {
                    document.getElementById('template_type').value = 'download';
                } else if (key === 'contact_auto_reply') {
                    document.getElementById('template_type').value = 'contact_auto_reply';
                } else if (key === 'booking_auto_reply') {
                    document.getElementById('template_type').value = 'booking_auto_reply';
                } else if (key === 'ebook_follow_up') {
                    document.getElementById('template_type').value = 'follow_up';
                } else if (key === 'welcome' || key === 'newsletter' || key === 'promotion') {
                    document.getElementById('template_type').value = 'campaign';
                } else if (key === 'password_reset' || key === 'verification' || key === 'receipt' || key === 'invoice') {
                    document.getElementById('template_type').value = 'verification'; // fallback or similar
                }
...
<body style="margin: 0; padding: 0; background-color: ${theme.backgroundColor}; font-family: ${theme.defaultFont};">`;
    ...
    function downloadHTML() {
        ...
        a.download = templateState.settings.name || 'new_template.html';
        a.click();
    }
